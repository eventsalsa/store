...
func (g *Generator) generateImports() string {
	...
	sb.WriteString("\t\"github.com/eventsalsa/store\"\n")
...
// This is useful for event handlers and projections that need to convert individual events.
...
func To%sV%d(streamType string, streamID string, e %s.%s, opts ...Option) (store.Event, error) {
	options := &eventOptions{}
	for _, opt := range opts {
		opt(options)
	}
...
func TestTypeHelpers(t *testing.T) {
	...
	// Test To helper
	esEvent, err := To%sV%d(streamType, streamID, domainEvent)
	if err != nil {
